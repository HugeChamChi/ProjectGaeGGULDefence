...
/// Awake()는 실제 인스턴스 생성 시 단 한 번만 호출되므로, 그 시점의 프리팹 원본 localScale/
/// localRotation을 BaseScale/BaseRotation으로 고정해 둔다. 이후 크기·회전을 적용할 때는 항상
/// ApplyScale()/BaseRotation 기준으로 다시 계산하도록 강제해, transform에 매번 곱연산해
/// 재사용될 때마다 값이 누적되는 버그(예: 기존 Projectile.Launch())를 원천 차단한다.
...
public abstract class BaseObject : MonoBehaviour
{
    ...
    /// <summary>Awake() 시점(프리팹 원본)의 localRotation. 회전 연출(FaceDirection 등)이 이 값을
    /// 기준으로 다시 계산되도록 공개한다.</summary>
