...
public class BossHpBarShake : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        if (_hpBar != null)
        {
            ...
            _hpBar.OnHpReset += ResetBaseline;
        }
        ResetBaseline();
    }
    ...
    {
        KillShake();
        _lastBucket = _hpBar != null && _hpBar.CurrentLine >= 0
            ? (_hpBar.LineCount - _hpBar.CurrentLine) / Mathf.Max(1, _lineInterval)
            : int.MinValue;
    }

    private void HandleLineChanged(int currentLine)
    {
        int bucket = (_hpBar.LineCount - currentLine) / Mathf.Max(1, _lineInterval);
        ...
        // 최대 체력부터 소진한 줄 기준. 100→99에서는 재생하지 않고 100→90에서 재생.
