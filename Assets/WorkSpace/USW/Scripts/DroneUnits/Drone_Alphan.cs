...
public class Drone_Alphan : ChiefUnit
{
    ...
    /// <inheritdoc />
    public override float SkillCooldownMultiplier => 1f - (DroneSelections?.Get(DroneSelectionKind.AlphanCooldown)?.Value ?? 0f);

    protected override void OnSkillFull()
    {
        if (unitData == null) return;
        _droneManager?.ApplyEmergencyBuffs();
        ...
        _droneManager
            .ExecuteRallyAsync(damagePerDrone, this.GetCancellationTokenOnDestroy(), DroneSelections?.Get(DroneSelectionKind.AlphanDoubleShot))
